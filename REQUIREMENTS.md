...
✅ **E2E 검증 완료(2026-07-04)**: MCP request_part(pin_header_254,7) → CI 생성(micromamba/conda-forge freecad+trimesh+scipy, ~1분!) → 게이트 → 봇 커밋 → 인라인 배포 → **/p/pin_header_254_7pin/ 라이브(53부품)**. 트러블슈팅 기록: ①apt에 freecad 없음(noble)→conda-forge ②trimesh가 scipy 요구 ③**배포 레이스**: generate-part와 deploy.yml이 다른 concurrency 그룹이라 옛 콘텐츠가 이길 수 있음→둘 다 `group: pages`로 직렬화.
...
6. **스케일 전환 (사용자 확정 2026-07-04, "이 정도 부품으로 되겠어?")**: 93개는 논지 검증용 시드일 뿐 — 스케일은 손배치가 아니라 메커니즘에서. 우선순위: ①**패밀리 파라메트릭 확장**(검증된 갭 패밀리의 실존 MPN 변형 — HT73xx 전압변형 등, config 한 줄=부품 하나) ②**검증-수입 조사**(라이선스 호환 오픈 라이브러리를 게이트 통과시켜 편입 — Espressif Apache-2.0 유력, 후보별 라이선스/규모/중복/품질 감사 후 착수 결정) ③온디맨드·기여(구축 완료, 트래픽 대기) ④배치 공장화. **①+②조사 착수(2026-07-04).**
