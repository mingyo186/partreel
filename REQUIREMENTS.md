...
| D 핀 능력 검증 | ST 핀 XML로 AF 기계 대조 | 🔴 다음 착수 후보 | 타이머/ADC 능력, ESP32 헤더 |
| E 깃 형상관리 | boardworks CI 푸시 게이트 | 🔴 미구현 | 그림 diff, rev 태그 릴리스, 거버 자산 |
| F 수치 출처 기록 | block.json에 출처 필드+게이트 | 🔴 미구현 | PDF 벡터 실측 자동 대조 |
