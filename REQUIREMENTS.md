...
**수입 확대 (사용자 확정 2026-07-05, "무료를 더 끌어쓰자 — 이름만 남기면 됨 + 저쪽엔 에이전트 전용 공간이 없다")**: 허용적 라이선스 라이브러리를 계속 수입 — 가치는 복사가 아니라 **에이전트-네이티브 승격**(부품별 API/MCP/번들/게이트/provenance/피드백 — 원 레포들엔 전무). 순서: ①**CERN Wave 0 착수** (~425: Crystals+LEMO; 시나리오 C=메타+에셋 Pages 내, R2는 Wave 2 전 별도 확인; verified-2D 등급=3D 없는 부품은 files에서 step/glb 제외+페이지 3D탭 숨김; GENERIC 3,324 제외; NRND는 수명주기 표기; 멀티유닛은 Wave 0 스킵 — docs/cern-import-plan.md) ②CDFER/JLCPCB MIT 라이브러리(EasyEDA 유래 지오메트리 출처 스팟체크 후) ③ai03 MX 스위치(MIT) ④SA 섹션(Espressif 등)은 별도 결정.
