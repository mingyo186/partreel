...
def guide_page():
  ...
  <h2>4. Browse PartReel inside KiCad (HTTP library)</h2>
  <p>KiCad 8+ can show the whole PartReel catalog in the <strong>symbol chooser</strong> as a live library:</p>
  <ol>
    <li>Download <a href="../../assets/partreel.kicad_httplib" download>partreel.kicad_httplib</a> and <a href="../../assets/PartReel.kicad_sym" download>PartReel.kicad_sym</a></li>
    ...
    <li>Open the symbol chooser — <code>PartReel_Live</code> categories appear (first open syncs for a few minutes), each part carrying its datasheet, page and direct download URLs in its fields</li>
  </ol>
  <p><em>Heads-up:</em> two KiCad-side limits, honestly stated. ① HTTP libraries carry metadata only (spec, not a PartReel limit) — placing a part inserts a placeholder symbol whose <code>PartReel</code> field links to the real symbol/footprint/3D downloads. ② KiCad pre-fetches every part one-by-one when the chooser first opens, so this library serves PartReel's <strong>curated, self-generated set</strong> (~450 parts) rather than the full 21,000+ catalog — search the full catalog at <a href="../../">partreel.com</a> or via the MCP server.</p>
  <p><a href="../../">← Home</a></p>
</main>"""
