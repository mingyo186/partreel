...
<link rel="stylesheet" href="{prefix}assets/style.css?v=7">
{head_extra}
</head>
<body>
<header class="topbar">
...
def part_page(meta, path):
  ...
  MCP: <code>{MCP_URL}</code> → <code>get_part("{pid}")</code>. See <a href="{DOMAIN}/llms.txt">/llms.txt</a> · <a href="{prefix}agents/">agent guide</a></p>
    ...
    scripts = f'<script type="module" src="{prefix}assets/part.js?v=13"></script>'
...
def guide_page():
  ...
  <p><strong>Easiest — KiCad's own Plugin and Content Manager.</strong> Register PartReel once and install/update from inside KiCad; it registers the symbol and footprint libraries for you:</p>
  <ol>
    <li>KiCad main window → <strong>Plugin and Content Manager</strong> → <strong>Manage…</strong> → <strong>+</strong></li>
    <li>Paste this repository URL: <code>https://partreel.com/pcm/repository.json</code> → OK</li>
    <li>Open the <strong>Libraries</strong> tab → <strong>PartReel Library</strong> → <strong>Install</strong> → <strong>Apply Pending Changes</strong></li>
    <li>Done — the parts are in your symbol chooser. When PartReel adds parts, PCM shows an update; one click refreshes everything.</li>
  </ol>
  ...
  <h3>Experimental: live HTTP library (not recommended)</h3>
  <p>KiCad 8+ can also pull part metadata over the network (<a href="../../assets/partreel.kicad_httplib" download>partreel.kicad_httplib</a>). It works, and the endpoint stays up for anyone already using it, but we do <strong>not</strong> recommend installing it alongside the offline files, for reasons that are inherent to KiCad's HTTP-library design rather than to PartReel:</p>
  <ul>
    <li><strong>Duplicate entries</strong> — parts served over HTTP also exist in your offline <code>PartReel</code> library, so they appear twice in the symbol chooser.</li>
    <li><strong>Startup pause</strong> — KiCad fetches every part one-by-one when the chooser first opens (about 20 seconds here), and repeats it after each restart.</li>
    <li><strong>Metadata only</strong> — the spec cannot deliver symbols, so <code>PartReel.kicad_sym</code> is required anyway; the HTTP library adds nothing you don't already have offline.</li>
    <li><strong>Network coupling</strong> — if this site is slow or unreachable, your schematic editor waits on it.</li>
  </ul>
  <p>Use the offline files above instead, and re-download them when you want newer parts.</p>
  <p><a href="../../">← Home</a></p>
</main>"""
...
adding it to the registry (how_to_contribute).""")
  ...
  <ul>
    ...
    <li>Tools: <code>search_parts(query)</code> · <code>get_part(id)</code> · <code>list_parts()</code> · <code>submit_part(...)</code> · <code>report_feedback(part_id, result, notes)</code> · <code>request_part(...)</code> · <code>how_to_contribute()</code></li>
    <li><strong>The loop</strong>: search → use → if missing, build it from the datasheet and <code>submit_part</code> — it is shared with every agent instantly (unverified), then promoted to verified by CI gates + review.</li>
    <li><strong>Please call <code>report_feedback</code> after using a part on a real board</strong> — it builds the part's field-proven trust score.</li>
  </ul>
  ...
  <p>Made a part the registry lacks? Share it <strong>instantly</strong> with the <code>submit_part</code> MCP tool — it becomes searchable and downloadable by every agent the moment it is accepted (labeled <em>staging/unverified</em>), and hourly CI gates promote it to verified via an auto-opened PR. The <a href="{GITHUB}" target="_blank" rel="noopener">GitHub</a> PR path remains for 3D models and edits. Spec: <a href="{GITHUB}/blob/main/CONTRIBUTING-AGENTS.md" target="_blank" rel="noopener">CONTRIBUTING-AGENTS.md</a>. Field reports (successes and problems) are welcome via <code>report_feedback</code>.</p>
  <p><a href="../">← Home</a></p>
</main>"""
